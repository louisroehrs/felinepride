...
public class ApiKeyTest extends AbstractResourceTest {
    ...
    @Test
    public void FPApiKey() {

        Response response = client.target(
                String.format("http://localhost:%d/graphql" , SUPPORT.getLocalPort()))
                .request()
                .header(HeaderParamNames.AUTHORIZATION, ApiKeyDirector.encodeApiKeyToBasic(FP_API_KEY))
                .get(Response.class);
...
/*
    ...
    public void graphqlSmokeTest () {
        try {
            Response response = client.target(
                    String.format("http://localhost:%d/graphql",SUPPORT.getLocalPort()))
                    .queryParam("query", URLEncoder.encode("{user(userName:\"lroehrs@felinepride.com\"){firstName}}","UTF-8"))
