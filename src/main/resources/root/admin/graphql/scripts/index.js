...
function setRowContent(item, row) {
    ...
    var doAttachButton = ""; 
    if (!(attachments && attachments.length > 0)) {
        doAttachButton = attachButton
    }
    row.innerHTML = innerHTML + doAttachButton + "</td>" + 
        "<td class = 'contentAction'><button class='viewGraphQLBtn' onclick='viewGraphQL(this)' title='view graphql'>GraphQL</span></td>"+
...
function viewGraphQL(deleteBtnNode) {
    window.location.href = "visualizer.html";
}

function graphiQL(deleteBtnNode) {
